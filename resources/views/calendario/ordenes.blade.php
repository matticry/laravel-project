...
@section('content')
    ...
    <div x-show="activeTab === 'ordenes'" class="mt-6">
        ...
        <div class="mb-4 mt-4 flex items-center space-x-4">
            ...
            <form action="{{ route('calendario.ordenes') }}" method="GET" class="flex space-x-2">
                <!-- Campo para código de empleado -->
                <div>
                    <label for="wo_order_code" class="block text-sm font-medium text-gray-700">Código de la órdenes:</label>
                    <input type="text" name="wo_order_code" autocomplete="off" id="code_emplo" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" placeholder="Filtrar por numero de reporte" value="{{ request()->get('wo_order_code') }}">
                </div>
                                ...
                                @can('workorders.destroy')
                                    <form action="{{ route('calendario.destroy', $workorder->wo_id) }}" method="POST"
                                        ...
                                        <button type="button"
                                                ...
                                                onclick="showModal('delete-modal-{{ $workorder->wo_id }}')">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                                 xmlns="http://www.w3.org/2000/svg">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <x-bladewind::modal
                                            ...
                                            ok_button_label=""
                                            cancel_button_label=""
                                            name="delete-modal-{{ $workorder->wo_id }}">
                                            <p>¿Estás seguro de que deseas eliminar esta orden de trabajo? Esta acción no se puede deshacer.</p>
                                            <div class="mt-4">
                                                <button type="button" class="bg-gray-300 text-gray-700 px-4 py-2 rounded mr-2" onclick="hideModal('delete-modal-{{ $workorder->wo_id }}')">Cancelar</button>
                                                <button type="button" class="bg-red-500 text-white px-4 py-2 rounded" onclick="document.getElementById('delete-form-{{ $workorder->wo_id }}').submit()">Sí, eliminar</button>
                                            </div>
                                        </x-bladewind::modal>
                                @endcan
                            </div>
                ...
                @endforeach
                    ...
                    <x-bladewind::modal
                        ...
                        @endif
                            ...
                            @if(isset($workorder))
                                ...
                                <form id="service-form" method="POST" enctype="multipart/form-data" action="{{ route('calendario.update', ['workOrderId' => $workorder->wo_id]) }}">
                                    ...
                                    <div class="overflow-y-auto max-h-[calc(90vh-100px)]">
                                        ...
                                        <div class="mt-4">
                                            <p id="total-display" class="text-right font-bold text-2xl text-gray-800 bg-gray-100 px-4 py-2 rounded-md shadow-sm">Total: $ <span id="total-value">0.00</span></p>
                                        </div>
                                        <input type="hidden" name="wo_total" id="wo_total" value="{{$workorder->wo_total}}">
                            ...
                            @else
                                <p>No se ha seleccionado ninguna orden de trabajo.</p>
                            @endif
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        var woTotal = document.getElementById('wo_total');
        var totalDisplay = document.getElementById('total-value');

        // Función para actualizar el total
        function updateTotal() {
            var value = parseFloat(woTotal.value) || 0;
        ...
        // Actualizar el total cuando cambie el valor del input
        woTotal.addEventListener('input', updateTotal);
    });
</script>
